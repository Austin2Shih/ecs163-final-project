...
export function initCountryCodeConversionMaps() {
  ISO3ToIdMap = Object.fromEntries(
    appState.data.countryCodeData.map(({ country_code_alpha3, country_id }) => [
      country_code_alpha3,
      +country_id,
    ])
  );
  ...
  ISO2ToIdMap = Object.fromEntries(
    appState.data.countryCodeData.map(({ country_code_alpha2, country_id }) => [
      country_code_alpha2,
      +country_id,
    ])
  );
  ...
  ISO3ToISO2Map = Object.fromEntries(
    appState.data.countryCodeData.map(
      ({ country_code_alpha3, country_code_alpha2 }) => [
        country_code_alpha3,
        country_code_alpha2,
      ]
    )
  ...
  IdToNameMap = Object.fromEntries(
    appState.data.countryCodeData.map(({ country_id, country_name }) => [
      +country_id,
      country_name,
    ])
  );

  NameToISO2Map = Object.fromEntries(
    appState.data.countryCodeData.map(
      ({ country_name, country_code_alpha2 }) => [
        country_name,
        country_code_alpha2,
      ]
    )
...
export function convertNameToISO2(name) {
  return NameToISO2Map[name];
}
