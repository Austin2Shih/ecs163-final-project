...
export default function displayBarChart(selectedCountry) {
  ...
  if (selectedCountry && disasterData) {
    countryDisasterData = disasterData.filter(
      (d) => d.ISO === selectedCountryISO3
    );
    console.log(
      `Found ${countryDisasterData.length} records using ISO3: ${selectedCountryISO3}`
    );
  }
  ...
  if (!selectedCountry) {
    ...
    console.log(
      'No country selected - showing world data:',
      countryDisasterData.length,
      'records'
    );
  } else {
    countryDisasterData = disasterData.filter(
  ...
  function processDisasterData(rawData) {
    ...
    rawData.forEach((disaster, index) => {
      ...
      if (!year || year < 1950 || year > 2030) {
        if (index < 5)
          console.log(
            `Record ${index}: Invalid year`,
            year,
            'from fields:',
            yearFields.map((f) => disaster[f])
          );
        return;
      }
      ...
      for (let field of costFields) {
        if (disaster[field] && !isNaN(parseFloat(disaster[field]))) {
          ...
          if (index < 5)
            console.log(
              `Record ${index}: Found cost ${cost} in field "${field}"`
            );
          break;
        }
      }
      ...
      if (cost > 0) {
        ...
        if (index < 5)
          console.log(
            `Record ${index}: Cost ${cost} -> ${costInBillions}B, Type: ${disasterType}`
          );

        //categorize disasters - only the three we want
        if (disasterType.includes('earthquake'))
          rangeData.earthquake += costInBillions;
        else if (disasterType.includes('flood'))
          rangeData.flood += costInBillions;
        else if (
          disasterType.includes('hurricane') ||
          disasterType.includes('cyclone') ||
          disasterType.includes('typhoon') ||
          disasterType.includes('storm')
        )
          rangeData.hurricane += costInBillions;
      }
    });

    console.log(
      `Processing summary: ${recordsProcessed} total, ${recordsWithValidYear} with valid years, ${recordsWithValidCost} with valid costs`
    );
  ...
  const disasterTypes = [
    { key: 'flood', name: 'Flood', color: '#2ca02c' }, // Green to match map
    { key: 'hurricane', name: 'Hurricane', color: '#1f77b4' }, // Blue to match map
    { key: 'earthquake', name: 'Earthquake', color: '#d62728' }, // Red to match map
  ];

  // Separate order for legend display
  const legendOrder = [
    { key: 'earthquake', name: 'Earthquake', color: '#d62728' }, // Red to match map
    { key: 'hurricane', name: 'Hurricane', color: '#1f77b4' }, // Blue to match map
    { key: 'flood', name: 'Flood', color: '#2ca02c' }, // Green to match map
